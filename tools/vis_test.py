...
from vis_tool import SaveDetectionHook

wekan_config = '../configs/rotated_fcos/rotated-fcos-le90_r50_fpn_1x_wavekan-RSAR.py'
wekan_resume = True
wekan_name = 'WEAttention-KAN-T05T05F'
wekan_show_thrs = 0.1
wekan_show_dir = 'vis_results'
...
def trigger_visualization_hook(cfg, args):
    ...
    if 'visualization' in default_hooks:
        ...
        # 注释或删除下面这行，以移除阈值过滤
        visualization_hook['score_thr'] = wekan_show_thrs  # <--- 删除或注释这行

        # --- 新增：配置visualizer以自定义线宽 ---
        # 确保visualizer存在，并设置线宽（例如设置为2）
        ...
        # 设置绘制参数
        cfg.visualizer['line_width'] = 2  # 将边框线宽设置为2像素
...
def main():
    ...
    save_hook = SaveDetectionHook(save_dir='detection_results')
    runner.register_hook(save_hook, priority='NORMAL')
    # start training
