...
from ..layers.kans import KANLayers_MLP
import numpy as np

INF = 1e8

def u_shape_weight(prob, alpha=2.0, beta=2.0):
    """
    prob: 概率值 [0, 1]
    返回: 权重，在prob=0和prob=1时最大，prob=0.5时最小
    """
    weight = alpha * (1 - prob)**beta + beta * prob**alpha
    return weight
...
class RotatedFCOSHead(FCOSHead):
    ...
    def __init__(self,
                 ...
                 loss_kan: OptConfigType = None,
                 loss_L1 = None,
                 loss_GD = None,
                 kan: bool = False,
                 nb_weight: bool = False,
                 nb_pkl: str = None,
                 **kwargs):
        ...
        if loss_kan is not None:
            self.loss_kan = MODELS.build(loss_kan)
        else:
            self.loss_kan = None
        ...
        if loss_GD is not None:
            self.loss_GD = MODELS.build(loss_GD)
        else:
            self.loss_GD = None
        ...
        if self.kan:
            self.kan_ang = KANLayers_MLP(in_features=in_channels, out_features=self.angle_coder.encode_size)
    ...
    def forward_single(self, x: Tensor, scale: Scale,
        ...
        if self.kan:
            angle_pred = self.kan_ang(reg_feat)
    ...
    def loss_by_feat(
        ...
        if self.nb_weight:
            labels, bbox_targets, angle_targets, nb_probs = self.get_targets_nb(
                all_level_points, batch_gt_instances, batch_img_metas)
        else:
        ...
        if self.nb_weight:
            flatten_nb_probs = torch.cat(nb_probs)
            flatten_nb_weight = torch.ones(flatten_cls_scores.shape[0], device=flatten_cls_scores.device)
            flatten_nb_weight[pos_inds] = u_shape_weight(flatten_nb_probs[pos_inds])
            #sss1 = flatten_nb_probs[pos_inds].cpu().numpy()
            #sss2 = flatten_nb_weight[pos_inds].cpu().numpy()
            #print('s')
            '''
            if self.nb_weight:
                flatten_nb_probs = torch.cat(nb_probs)
                flatten_nb_weight = 1/flatten_nb_probs
                if len(pos_inds) > 1:
                    norm_weight = torch.sum(flatten_nb_weight[pos_inds])
                    flatten_nb_weight[pos_inds] = flatten_nb_weight[pos_inds] / norm_weight  * len(pos_inds)
                #flatten_nb_probs = flatten_nb_probs[pos_inds]
            '''
        ...
        loss_cls = self.loss_cls(
            flatten_cls_scores, flatten_labels, avg_factor=num_pos, weight=flatten_nb_weight)
       # boxes.append([x, y, w, h, angle / 180 * np.pi])
        ...
        if len(pos_inds) > 0:
            ...
            loss_bbox = self.loss_bbox(
                ...
                weight=pos_centerness_targets * flatten_nb_weight[pos_inds],
        ...
        if self.loss_angle:
            ...
            loss_kan = self.loss_kan(self.kan_ang.fc1.kan_layer.regularization_loss()+
                                     self.kan_ang.fc2.kan_layer.regularization_loss())
            losses = dict(
                loss_cls=loss_cls,
                loss_bbox=loss_bbox,
                loss_centerness=loss_centerness,
            ...
            losses = dict(
                ...
                loss_centerness=loss_centerness)
        if self.loss_GD:
            loss_GD = self.loss_GD(
                pos_decoded_bbox_preds,
                pos_decoded_target_preds,
                weight=pos_centerness_targets * flatten_nb_weight[pos_inds],
                avg_factor=centerness_denorm)
            losses['loss_GD'] = loss_GD
        if self.loss_L1:
    ...
    def get_targets_nb(
        self, points: List[Tensor], batch_gt_instances: InstanceList, batch_img_metas: List[Dict]
    ) -> Tuple[List[Tensor], List[Tensor], List[Tensor]]:
        """Compute regression, classification and centerness targets for points
        in multiple images.
        Args:
        ...
        # get labels and bbox_targets of each image
        labels_list, bbox_targets_list, angle_targets_list, nb_probs_list = multi_apply(
            self._get_targets_single_nb,
        ...
        nb_probs_list = [
            nb_probs.split(num_points, 0)
            for nb_probs in nb_probs_list
        ]
        ...
        for i in range(num_levels):
            ...
            angle_targets = torch.cat(
                [angle_targets[i] for angle_targets in angle_targets_list])
            nb_probs = torch.cat(
                [nb_probs[i] for nb_probs in nb_probs_list])
            if self.norm_on_bbox:
                bbox_targets = bbox_targets / self.strides[i]
            concat_lvl_bbox_targets.append(bbox_targets)
            concat_lvl_angle_targets.append(angle_targets)
            concat_lvl_nb_probs.append(nb_probs)
        return (concat_lvl_labels, concat_lvl_bbox_targets,
                concat_lvl_angle_targets, concat_lvl_nb_probs)

    def _get_targets_single_nb(
            self, gt_instances: InstanceData, batch_img_metas: Dict,
            points: Tensor,
            regress_ranges: Tensor,
        ...
        if num_gts == 0:
            ...
            return gt_labels.new_full((num_points,), self.num_classes), \
                gt_bboxes.new_zeros((num_points, 4)), \
                gt_bboxes.new_zeros((num_points, 1)), \
                gt_bboxes.new_ones((num_points,))  # 负样本概率为1
        ...
        # 计算朴素贝叶斯概率
        nb_probs = gt_bboxes.new_ones((num_points,))  # 默认概率为1（负样本）

        # 找出正样本（有对应真实目标的点）
        pos_mask = min_area != INF
        if pos_mask.any() and hasattr(self, 'nb_model') and self.nb_model is not None:
            # 获取正样本对应的真实边界框信息
            pos_indices = torch.where(pos_mask)[0]
            pos_min_area_inds = min_area_inds[pos_mask]
            scale_factor = batch_img_metas['scale_factor']

            # 获取正样本对应的真实边界框
            pos_gt_bboxes = gt_bboxes[pos_indices, pos_min_area_inds]

            # 提取几何特征：面积、角度、长宽比、长度、宽度
            # gt_bboxes格式: [cx, cy, w, h, angle]
            # 修正长宽提取逻辑
            pos_w = pos_gt_bboxes[:, 2] / scale_factor[0]  # 第一个尺寸
            pos_h = pos_gt_bboxes[:, 3] / scale_factor[1] # 第二个尺寸
            pos_angle = pos_gt_bboxes[:, 4]  # 角度

            # 确定长度和宽度（长边为length，短边为width）
            pos_length = torch.maximum(pos_w, pos_h)  # 长边作为长度
            pos_width = torch.minimum(pos_w, pos_h)  # 短边作为宽度

            # 计算面积和长宽比
            pos_area = pos_length * pos_width
            pos_aspect_ratio = pos_length / pos_width

            # 转换为numpy用于NB模型预测
            pos_area_np = pos_area.cpu().numpy()
            pos_angle_np = pos_angle.cpu().numpy()
            pos_aspect_ratio_np = pos_aspect_ratio.cpu().numpy()
            pos_length_np = pos_length.cpu().numpy()
            pos_width_np = pos_width.cpu().numpy()

            # 使用NB模型预测概率
            nb_probs_np = np.ones(len(pos_indices))  # 默认概率为1
            ...
            if len(pos_indices) > 0:
                ...
                # 批量预测概率
                batch_probabilities = self.nb_model.predict_proba(X_raw=batch_features)

                # 向量化获取每个样本对应真实类别的概率
                gt_labels_np = labels[pos_indices].cpu().numpy()
                nb_probs_np = batch_probabilities[np.arange(len(pos_indices)), gt_labels_np]
                #nb_probs_np = np.clip(nb_probs_np, 0.5, 1.0) # 设置一个下限
                #nb_probs_np = np.clip(nb_probs_np, 0.1, 1.0)  # 设置一个下限 新改的 0.5-0.1 用在fullp 0.1 不行
                #nb_probs_np = np.clip(nb_probs_np, 0.3, 1.0)  # 设置一个下限 新改的 0.5-0.1 用在fullp 0.3 不行
                #nb_probs_np = np.clip(nb_probs_np, 0.7, 1.0)  # 设置一个下限 新改的 0.5-0.1 用在fullp 0.7 差一点
                #nb_probs_np = np.clip(nb_probs_np, 0.6, 1.0)  # 设置一个下限 新改的 0.5-0.1 用在fullp 0.3 不行

            # 将概率转换回tensor
            nb_probs[pos_indices] = torch.from_numpy(nb_probs_np).to(
                nb_probs.device, dtype=nb_probs.dtype)

        # 负样本标签设为self.num_classes
        labels[min_area == INF] = self.num_classes

        return labels, bbox_targets, angle_targets, nb_probs
